...
def rasl_probe_blast(read_file_path, aligner_path, db_path, print_on=False):
    ...
    cmd_str = "{aligner_path}/blastn -task blastn-short -query {read_file_path} -evalue 1e-6 {wordsize_db} -max_target_seqs 1 -strand 'plus' -xdrop_gap 7 -num_threads 12 -outfmt '6 -outfmt qseqid -outfmt mismatch -outfmt evalue -outfmt sseqid -outfmt qlen -outfmt length -outfmt qseq -outfmt qstart -outfmt sseq -outfmt sstart -outfmt send' >{write_path}".format(
        aligner_path=aligner_path,
        read_file_path=read_file_path,
        wordsize_db=wordsize + db,
...
def get_star_alignments(read_fq_path, aligner_path, db_path, print_on=False, n_jobs=1, offset_5p=16, offset_3p=16):
    ...
    cmd = [aligner_path + '/STAR', '--genomeDir', db_path,
        '--readFilesIn', read_fq_path, '--runThreadN',
        str(n_jobs), '--seedSearchStartLmax 1', '--seedPerReadNmax',
        str(15000), '--scoreDelOpen -1', '--scoreDelBase -2',
        '--scoreInsOpen -1', '--scoreInsBase -2', '--outFilterMultimapNmax 100',
        '--outFilterMatchNminOverLread 0.7', '--scoreGapNoncan -1', '--outFilterScoreMinOverLread 0.7',
        '--clip5pNbases', str(offset_5p), '--clip3pNbases',
        str(offset_3p), '--outStd SAM']
...
def swalign_df(ref, query):
    ...
    align_series = pd.Series([ref, query, aligned.r_pos,
        aligned.r_end, aligned.q_pos, aligned.q_end,
...
def mp_swalign(align_probes):
    ...
    for q_probe in q_probes:  # Iterate through reference probes
        if q_probe == ref:
            # Only consider non-self alignments
            continue
